...
esv_grid = arcpy.GetParameterAsText(0)
ltypes_grid = arcpy.GetParameterAsText(1)
clip_feature = arcpy.GetParameterAsText(2)
esv_out_table_name = arcpy.GetParameterAsText(3)
ltypes_out_table_name = arcpy.GetParameterAsText(4)

if arcpy.GetParameterAsText(5) != '':
	out_gdb = arcpy.GetParameterAsText(5)
else:
	out_gdb = arcpy.env.scratchWorkspace

esv_clip = 'in_memory/ESV_clip'
esv_out_table = out_gdb+'/'+esv_out_table_name
ltypes_clip = 'in_memory/LTypes_clip'
ltypes_out_table = out_gdb+'/'+ltypes_out_table_name

arcpy.env.overwriteOutput=True

# Check to ensure only a single development outline is selected
if not arcpy.Describe(clip_feature).FIDSet.split('; ')[0]:
	arcpy.AddError('No clip feature selected. Select a feature and re-run.')
elif len(arcpy.Describe(clip_feature).FIDSet.split('; ')) > 1:
	arcpy.AddError('Multiple clip features selected. Select a single feature and re-run.')
else: 
	### Section controlling Ecosystem Service summary ###
	# Clip ESV raster and store in-memory
	arcpy.Clip_management(esv_grid,'#',esv_clip,clip_feature,None,'ClippingGeometry')
	...
	esv_value_list = listBuild(5)

	# Multiple each field by cell count
	with arcpy.da.SearchCursor(esv_clip,esv_fields) as cursor:
		esv_value_list[0]='Dollar Value'
		for row in cursor:
			esv_value_list[1]+=(row[esv_fields.index('FloodControl_DollarValue')]*row[esv_fields.index('Count')])
			esv_value_list[2]+=(row[esv_fields.index('GroundwaterRecharge_DollarValue')]*row[esv_fields.index('Count')])
			esv_value_list[3]+=(row[esv_fields.index('WaterPurification_DollarValue')]*row[esv_fields.index('Count')])
			esv_value_list[4]+=round((float(row[esv_fields.index('CarbonStorage_DollarValue')])*float(row[esv_fields.index('Count')])),0)
			esv_value_list[5]+=round((float(row[esv_fields.index('CombinedServices_DollarValue')])*float(row[esv_fields.index('Count')])),0)
	...
	# Create output table
	arcpy.CreateTable_management(out_gdb,esv_out_table_name)

	# Add fields to out_table
	arcpy.AddField_management(esv_out_table,'type','TEXT','50')
	esv_out_field_list = ['FloodControl','GroundwaterRecharge','WaterPurification','CarbonStorage','AggregateServices']
	...
	arcpy.Clip_management(ltypes_grid,'#',ltypes_clip,clip_feature,None,'ClippingGeometry')
	...
	sum_esv_table_view = arcpy.mapping.TableView(esv_out_table)
	sum_ltypes_table_view = arcpy.mapping.TableView(ltypes_out_table)
	arcpy.mapping.AddTableView(df, sum_esv_table_view)
	arcpy.mapping.AddTableView(df, sum_ltypes_table_view)
	arcpy.RefreshTOC()