...
for (i in 1:length(counties))
{
  ...
  # Subset top ten
  sub_spp <- cty_spp[1:10,]

  write.csv(sub_spp,paste(file_path,counties[i],'_top10results_rank_',species,'.csv',sep=''))

  colnames(sub_spp)[c(10:23)] <- c('acreage','bird_count','Flood','Grndwtr','WaterPur','CarbonStor','Aggregate','acreage','bird_count','Flood','Grndwtr','WaterPur','CarbonStor','Aggregate')
  
  output <- data.frame(sub_spp[1,c(1:2,10:16)])
  colnames(output) <- c("county","patch_id","acreage","bird_count","Flood","Grndwtr","WaterPur","CarbonStor","Aggregate")

  output2 <- data.frame(sub_spp[1,c(1:2,17:23)])
  ...
  for (j in 2:10)
  {
    output <- rbind(output,sub_spp[j,c(1:2,10:16)])
    output <- rbind(output,sub_spp[j,c(1:2,17:23)])   
  }

  output <- data.frame(county=rep(counties[i],20),output[,c('patch_id')],status=rep(c('protected','unprotected'),10),output[,3:9])

  output$bird_count<-as.numeric(output$bird_count)
  cty_spp$bird_count<-as.numeric(cty_spp$bird_count)
  output$bird_percent<-output$bird_count/(sum(cty_spp$bird_count))

  output <- output[,c(1:5,11,6:10)]

  colnames(output) <- c("County","Patch ID","Status","Acreage","Number of Birds","Percentage of Birds","Flood Control","Groundwater Recharge","Water Purification","Carbon Sequestration","All Services")
  
  output$Species <- species

  output <- output[,c(12,1:11)]
