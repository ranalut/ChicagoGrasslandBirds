...
for (i in 1:length(data.yrs))
{
	...
	nass.file.names <- c(nass.file.names,paste(nass.path,data.yrs[i],'_nass_clump_30m.tif',sep=''))
	# print(nass.file.names)
	temp <- stack(nass.file.names)
	# print(temp)
	nass.data[[i]] <- extract(temp,unique.pts)
	colnames(nass.data[[i]]) <- c(paste(nass.var,'.',rep(radius,each=length(nass.var)),sep=''),'patch.cells')
